  ...
  const catalog = {
    nodes: {
      ...
      domain: {
        ...
        relation: { text: "Glossary와 별개 축이다. 같은 Asset에 의미(Term)와 소유(Domain)가 동시에 붙는다.", chain: ["TAX_EXMP_FLG", "“세금면제” 개념 + Finance Domain 소속"], to: ["subdomain", "asset"] },
      },
      subdomain: {
      ...
      asset: {
        ...
        example: "같은 카탈로그가 TAX_EXMP_FLG(컬럼)도, 그걸 쓰는 “면세 대출 현황” 대시보드도, 둘 사이 ETL 쿼리도 모두 Asset으로 관리한다. 그래서 컬럼이 바뀌면 그 컬럼을 쓰는 대시보드까지 추적된다.",
      ...
      lineage: {
        ...
        lead: "Asset에서 Asset으로 데이터가 어떻게 흐르는지 기록한다.",
        facts: [["분류 전파", "Lineage를 따라 downstream Asset까지 분류가 번진다."]],
        example: "TAX_EXMP_FLG(원천) → 세금계산 모듈 → TAX_SUMMARY_DT(파생). 원천에 붙은 PII가 파생 컬럼으로 후보 전파된다.",
    ...
    propagation: {
      title: "분류 전파 — 두 단계",
      steps: [
        { node: "“Email” Term", tag: "PII", note: "분류를 가진 개념", kind: "term" },
        { node: "users.email", tag: "PII 확정", note: "직접 Link된 Asset · 즉시 확정", kind: "confirmed" },
        { node: "users_clean.email_hash", tag: "PII 후보", note: "Lineage 파생 · 검토 대상", kind: "candidate" },
      ],
    },
  };
